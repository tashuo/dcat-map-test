...
<head>
    ...
    <script charset="utf-8" src="https://map.qq.com/api/js?v=2.exp&key=2CABZ-2VOCB-56SUS-N2UB7-CZOPE-WFBLV"></script>
    <style type="text/css">
        * {
            margin: 0px;
            padding: 0px;
...
{{--            <div id="btn_search" class="btn">搜索</div>--}}
            <div class="search_c">
                <input id="zuobiao" type="text" class="search_t" />
            </div>
        </div>
    </div>
    <div id="main">
...
<script type="text/javascript">
    ...
    qq.maps.event.addListener(map, "click", function (e) {
        ...
        document.getElementById("zuobiao").value = coordinate;

        url3 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?location=" + e.latLng.getLat() + "," + e.latLng.getLng() + "&key=2CABZ-2VOCB-56SUS-N2UB7-CZOPE-WFBLV&output=jsonp&&callback=?");
    ...
    //     url = encodeURI("https://apis.map.qq.com/ws/place/v1/search?keyword=" + value + "&boundary=region(" + query_city + ",0)&page_size=9&page_index=1&key=2CABZ-2VOCB-56SUS-N2UB7-CZOPE-WFBLV&output=jsonp&&callback=?");
    ...
    function setFlagClicked(arr, index) {
        each(markerArray, function (n, ele) {
            if (n == index) {
                ...
                url3 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?location=" + latLng.getLat().toFixed(6) + "," + latLng.getLng().toFixed(6) + "&key=2CABZ-2VOCB-56SUS-N2UB7-CZOPE-WFBLV&output=jsonp&&callback=?");
                $.getJSON(url3, function (result) {
                    if (result.result != undefined) {
                        parent.setCoordinateArea(result.result.address);
                        document.getElementById("btn_content").value = result.result.address;
    ...
    city.onclick = function (e) {
        ...
        if (target.className == "city_name") {
            ...
            url2 = encodeURI("https://apis.map.qq.com/ws/geocoder/v1/?region=" + curCity.children[0].innerHTML + "&address=" + curCity.children[0].innerHTML + "&key=2CABZ-2VOCB-56SUS-N2UB7-CZOPE-WFBLV&output=jsonp&&callback=?");
    ...
    $(".search_t").autocomplete({
        source: function (request, response) {
            url4 = encodeURI("https://apis.map.qq.com/ws/place/v1/suggestion/?keyword=" + request.term + "&region=" + curCity.children[0].innerHTML + "&key=2CABZ-2VOCB-56SUS-N2UB7-CZOPE-WFBLV&output=jsonp&&callback=?");
